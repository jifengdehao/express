...
module.exports = {
	...
	useradd: function (req, res, next) {
		pool.getConnection(function(err, connection) {
			...
			// 建立连接，向表中插入值
			// 'INSERT INTO user(id, name, age, pwd) VALUES(0,?,?,?)',
			connection.query($sql.userinsert, [param.name, param.age, param.pwd], function(err, result) {
				if(result) {
					result = {
						code: 200,
						msg:'增加成功'
    ...
    userupdate: function (req, res, next) {
        // update by id
        // 为了简单，要求同时传name和age,pwd,id参数
        pool.getConnection(function(err, connection) {
			// 获取前台页面传过来的参数
			var param = req.query || req.params; 			
			connection.query($sql.userupdate, [param.name,param.age,param.id,param.pwd], function(err, result) {
				if(result) {
					result = {
						code: 200,
						msg:'修改成功'
